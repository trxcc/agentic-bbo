...
如果你想用自己上传的数据包覆盖默认数据，可以把 `BBO_BO_TUTORIAL_SOURCE_ROOT=/path/to/source_root` 指向新的根目录；像 `examples/HEA/data/oracle_data.xlsx` 这样的路径都会相对于那个根目录解释。
...
运行时依赖主要是 `pandas`、`openpyxl` 和 `scikit-learn`。
